...
uint8_t MPU6050_ReadByte(uint8_t regAddr) // 读取寄存器
{
    uint8_t res = 0;
    i2c_status_type i2c_status;
    I2C_BusLock();
...
uint8_t MPU6050_SetLPF(u16 lpf) // 设置低通滤波器 ，lpf为低通滤波器截止频率，单位为Hz
{
    ...
    if (lpf >= 188)
        ...
        data = 6;
    return MPU6050_WrReg(MPU_CFG_REG, data);
}

uint8_t MPU6050_GetAcc(short *ax, short *ay, short *az) // 获取加速度值
{
    uint8_t buf[6], res;
    res = MPU6050_ReadLen(MPU6050_ACCEL_XOUT_H, 6, buf);
    if (res == 0) {
...
uint8_t MPU6050_GetGyro(short *gx, short *gy, short *gz) // 获取陀螺仪值
{
    ...
    res = MPU6050_ReadLen(MPU6050_GYRO_XOUT_H, 6, buf);
    if (res == 0) {
...
short MPU6050_GetTemp(void) // 获取温度值
{
    ...
    MPU6050_ReadLen(MPU6050_TEMP_OUT_H, 2, buf);
    raw = ((u16)buf[0] << 8) | buf[1];
    temp = 36.53 + ((double)raw) / 340;
    return temp * 100;
}

uint8_t MPU6050_SetRate(u16 rate) // 设置采样率 ，rate为采样率，单位为Hz
{
    uint8_t data;
    if (rate > 1000)
        rate = 1000;
...
{
    uint8_t res;
    MPU6050_WrReg(MPU6050_PWR_MGMT_1, 0X80); // reset 6050
    delay_ms(100);
    MPU6050_WrReg(MPU6050_PWR_MGMT_1, 0X00); // wakeup 6050

    MPU6050_SetGyroFsr(3); // Gyro:+-2000dps
    ...
    /*
        ...
        MPU6050_WrReg(MPU6050_SMPLRT_DIV, 0x09);//
        MPU6050_WrReg(MPU6050_CONFIG, 0x06);//
        MPU6050_WrReg(MPU6050_GYRO_CONFIG, 0x18);//
        MPU6050_WrReg(MPU6050_ACCEL_CONFIG, 0x18);//
    */
}
