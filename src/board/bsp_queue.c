...
typedef struct {
    ...
    volatile uint8_t Length; // total length of queue
    volatile uint8_t Head;   // head position (index)
    volatile uint8_t Tail;   // tail position (index)
    volatile BYTE *Data;
} QueueImp;
